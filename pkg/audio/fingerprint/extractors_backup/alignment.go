...
func (la *LightweightAlignment) crossCorrelateManual(signal1, signal2 []float64) (float64, float64) {
	...
	for delay := -maxSearchDelay; delay <= maxSearchDelay; delay++ {
		...
		if corr > 0.01 { // Lower threshold - track any correlation > 1%
			correlationPeaks = append(correlationPeaks, struct {
				offset int
				corr   float64
			}{delay, corr})
		}
	...
	// DEBUG: Show top correlation peaks
	fmt.Printf("DEBUG CrossCorr: significant peaks found: %d\n", len(correlationPeaks))
	for i, peak := range correlationPeaks {
		if i < 5 { // Show top 5 peaks
			peakSeconds := float64(peak.offset) / la.FeatureRateHz
			fmt.Printf("  Peak %d: offset=%.2fs, correlation=%.4f\n", i+1, peakSeconds, peak.corr)
		}
	}
